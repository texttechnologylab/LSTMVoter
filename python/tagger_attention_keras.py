...
def createData():
    ...
    # embeddings_matrix,embeddings_index = load_embeddings("/home/staff_homes/ahemati/projects/biocreative/data/embeddings/glove.840B.300d.filtered.txt")
    # features_train,idxs_train,max_len = load_file("/home/staff_homes/ahemati/projects/biocreative/lstmVoter/data/train1.conll","\t",embeddings_index,considerFeatures=[5,6,8,9,10,11])
    # features,idxs,max_len = load_file("/home/staff_homes/ahemati/projects/biocreative/lstmVoter/data/small.conll","\t",embeddings_index,considerFeatures=[5,6,8,9,10,11],max_len=max_len,idxs=idxs_train)
    ...
    trainFile = "/home/staff_homes/ahemati/projects/biocreative/data/CHEMDNER/secound_step/development_taged_by_multitagger.conll"
    testFile = "/home/staff_homes/ahemati/projects/biocreative/data/CHEMDNER/secound_step/evaluation_taged_by_multitagger.conll"
    weights_path = "chemdner_train_on_dev_100_mit_attention_with_marmot_and_stanford_attention_hyperopt"
    checkFile(trainFile)
    checkFile(testFile)
...
#         merge_lstm1 = Dense(100, activation="tanh")(merge_lstm1)
    ...
    def train(model, features_train,embeddings_matrix,idxs,weights_path):
        ...
        model.fit(inputs, np.array(features_train["output"]), batch_size=128, epochs=50, verbose=1 
    ...
    model.save_weights("/home/staff_homes/ahemati/projects/biocreative/lstmVoter/models/hyperas/model.h5."+str(f1))
    print str(trials._trials[-1])
    with open("/home/staff_homes/ahemati/projects/biocreative/lstmVoter/models/hyperas/model.json."+str(f1), "w") as json_file:
...
if __name__ == '__main__':
    ...
    #print(X_train)
    #utils_eval.data_batch()
    #words_train, char_train, feature_train, pv_train, y_train, words_test, char_test, feature_test, pv_test, y_test, max_len, embedding_matrix, char_idx, word_idx, test_words, pos_idx = utils.data_batch()
    #create_model(words_train,char_train,feature_train, pv_train, y_train, words_test, char_test, feature_test,pv_test, y_test, max_len, None, char_idx, word_idx, test_words)
    #print X_train
    #print Y_train
    #print X_train
...
#     
#     features_test,idxs,max_len = load_file(testFile,"\t",embeddings_index,considerFeatures=consideredFeatures,max_len=max_len,idxs=idxs)
#     precision, recall, f1,output_return = test(model,features_train,features_test,idxs,sys.argv[2])
#          
