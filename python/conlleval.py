...
def countChunks(fileIterator):
    ...
    for line in fileIterator.splitlines():
...
def evaluate(correctChunk, foundGuessed, foundCorrect,correctTags, tokenCounter):
    # sum counts
...
def global_evaluate(text):
    correctChunk, foundGuessed, foundCorrect, correctTags, tokenCounter = countChunks(text)

    # compute metrics and print
    return evaluate(correctChunk, foundGuessed, foundCorrect,correctTags, tokenCounter)
